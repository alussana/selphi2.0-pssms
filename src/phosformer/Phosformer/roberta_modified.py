...
"""This has been modified to be used with Phosformer.
   Contains code from "configuration_roberta.py", "roberta_custom.py", and "pytorch_utils.py"
"""
...
class RobertaClassificationHead(nn.Module):
    ...
    def forward(self, features, **kwargs):
        x = features[:, 6, :] # modified for Phosformer: use residue embedding of the potential phosphosite
        x = self.dropout(x)
        x = self.dense(x)
        x = torch.tanh(x)
        x = self.dropout(x)
