...
def filter_dict(d:dict, m:int):
    return {k: v for k, v in d.items() if len(v) > m}
...
def main():
    selphi_k_t_tsv = sys.argv[1]
    out_pdf = sys.argv[2]
    selphi_k_p_tsv = sys.argv[3]
    kinase_family = sys.argv[4]
    minphos = int(sys.argv[5])
    distance_matrix_tsv = sys.argv[6]
    kinase_annotations_csv = sys.argv[7]
    """
    ...
    selphi_k_t.columns = ["Term","Kinase"]
    ...
    # lookup table of kinase specificity class
    kinase_specificity_class = pd.read_csv(kinase_annotations_csv, index_col=0)
    kinase_specificity_class.columns = ["Kinase","Specificity_sourced","Specificity"]
    kinase_specificity_class = kinase_specificity_class[["Kinase", "Specificity"]].drop_duplicates().reset_index()


    # make dictionary of terms sets for each kinase
    kinase_phospho_sets = selphi_k_t.groupby('Kinase')['Term'].apply(set).to_dict()
    kinase_phospho_sets = filter_dict(kinase_phospho_sets, minphos)
    ...
    distance_df.to_csv(distance_matrix_tsv, sep="\t")
